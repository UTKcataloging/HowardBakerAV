# OpenRefine Template for Howard Baker Audiovisual Collection (Side A)
...
```
<mods>
<identifier type="pid">{{cells['PID_A'].value}}</identifier>
<identifier type="local">{{cells['adminDB_A'].value}}</identifier>
...
<extent>{{cells['SideA'].value}}</extent></physicalDescription>
